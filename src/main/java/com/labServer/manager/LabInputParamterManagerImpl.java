...
public class LabInputParamterManagerImpl implements LabInputParamterManager {
   ...
   * @param labInputParamter
   */
  public void addListItemsToSum(@Param("list")List<LabInputParamter> list) {
  ...
  public void addListItemsToDiff(@Param("list")List<LabInputParamter> list){
    SqlSession sqlSession = MyBatisUtil.getSqlSession();
  ...
  public void addLabInputParamterByDynamicTableName(LabInputParamter labInputParamter) {
    SqlSession sqlSession = MyBatisUtil.getSqlSession();
    String statement =
        "com.labServer.mapping.LabInputParamterMapper.addInputParamsByDynamicTableName";
    sqlSession.insert(statement, labInputParamter);
    sqlSession.commit();// 手动提交事务
    sqlSession.close();
  }
  ...
  public Double getAVGInputTemperatureByCreatedOn(LabInputParamter labInputParamter,
    ...
    Double avgInputTemperature =
        labInputParamterMapper.findAVGInputTemperature(labInputParamter, inputTable);
    avgInputTemperature =
        OptimizedTemp(labInputParamter.getInputTemperature(), avgInputTemperature);
    sqlSession.commit();
    sqlSession.close();
